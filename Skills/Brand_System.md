...
The Clinic of AI logo is a two-dimensional cube outlined in black ink, with an orange dot in its center, set against a beige background and accented with a soft-lemon glow. The cube reads as a contained, structured object — a unit, a system, something measurable — and the orange dot functions as a focal point, a diagnostic marker, the thing being identified or examined inside the system. This logic carries through the entire visual system: structured neutral environments hold focused points of orange interest, with soft-lemon surfaces used as supporting warmth and contrast. The overall aesthetic is editorial, minimal, and quietly confident. Visuals should feel closer to a high-end industry publication or an architectural firm's portfolio than to a SaaS company's marketing site. Mood is calm, precise, and grounded — never frantic, never speculative, never futuristic.

Color System

The brand palette is three colors: **beige, orange, and soft lemon**. Each plays a distinct role and the entire visual identity rests on the relationship between them.

**Beige** is the canvas. The dominant background color is a warm off-white beige, #fff8f3, used as the primary surface for most assets. Tinted variants — #fff5e8, #fff2e2, and #efe0cd — provide gentle layering for cards, container surfaces, and content modules. Beige reads as warm paper, never as cold neutral.

**Orange** is the accent and the action color. It appears in two values: #a14000 is the deeper, more serious burnt orange used for emphasis in professional contexts and as the deep-text variant when ink is needed for body type; #ff7a32 is the brighter focal variant used sparingly as the diagnostic marker, the highlighted node, the call-to-action, the single moment of attention in a composition. Orange is always treated as a precision element, never as a fill or a wash. It appears in small concentrations: a single dot, a highlighted callout, a thin underline, an emphasized word, a primary CTA.

**Soft lemon** (#FFF59D, with a softer #FFFACD variant for very light surfaces) is the supporting warmth and gentle-contrast surface color. It carries decorative emphasis and section-band weight where orange would be too loud — hero-band accents, the calm surface behind closing CTAs, featured cards, decorative blobs in card corners. Soft lemon acts as the muted alternate surface alongside beige — quieter than canary yellow, never aggressive. It must never be used for type; it is always a surface, paired with black ink for legibility.

Together, the three colors form the visual identity: a beige field with focused orange interest and soft-lemon supporting warmth. Type and structural strokes are rendered in pure black ink so the editorial clarity is preserved. Forbidden colors include purple-to-pink SaaS gradients, glowing electric blues, cyberpunk magenta or cyan, and any "glow-on-dark" treatments. Light mode (beige background) is the primary mode. Type and structural strokes use pure black ink that sits cleanly inside the beige + orange + soft-lemon palette without competing with it.
...
Color and aesthetic errors: No purple-to-pink SaaS gradients, no glow-on-dark treatments, no oversaturated photography, no gradient-overload backgrounds, no electric blue corporate-tech tones. The palette is beige, orange, and soft lemon — and it stays that way.
