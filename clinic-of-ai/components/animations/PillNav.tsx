...
// Primary (soft lemon #FFF59D) → pill background
// Secondary (#a14000)→ active indicator dot
// On-surface black   → pill text / hover text

const COAI_BASE        = 'rgba(255,248,243,0.85)' // surface at 80% — glassmorphism
const COAI_PILL        = '#FFF59D'                 // soft lemon pill
const COAI_PILL_TEXT   = '#000000'                 // black on soft lemon
const COAI_HOVER_TEXT  = '#000000'                 // stays black on hover reveal
...
const PillNav: React.FC<PillNavProps> = ({
  ...
  baseColor = COAI_BASE,
  pillColor = COAI_PILL,
  hoveredPillTextColor = COAI_HOVER_TEXT,
  pillTextColor = COAI_PILL_TEXT,
  onMobileMenuClick,
  ...
  const cssVars = {
    ...
    ['--active-dot']: '#a14000',   // secondary orange per DESIGN.md
    ['--nav-h']: '42px',
    ['--logo']: '36px',
    ['--pill-pad-x']: '18px',
    ['--pill-gap']: '3px',
    ...
    return (
      <>
        <span
          ...
          style={{ background: 'var(--pill-bg, #FFF59D)', willChange: 'transform' }}
          aria-hidden="true"
        ...
        {isActive && (
          <span
            ...
            style={{ background: 'var(--active-dot, #a14000)' }}
            aria-hidden="true"
          />
        )}
      </>
        ...
        <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
          {items.map(item => {
            ...
            return (
              <li key={item.href}>
                {isExternalLink(item.href) ? (
                  <a href={item.href} className={linkClasses} style={defaultStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={() => setIsMobileMenuOpen(false)}>
                    {item.label}
                  </a>
                ) : (
                  <Link href={item.href} className={linkClasses} style={defaultStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={() => setIsMobileMenuOpen(false)}>
                    {item.label}
                  </Link>
                )}
              </li>
