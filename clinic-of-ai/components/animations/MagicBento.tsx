      ...
      <div className="bento-section grid gap-2 p-3 max-w-[54rem] select-none relative" style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            ...
            if (enableStars) {
              return (
                <ParticleCard key={index} className={baseCardClass} style={cardStyle(card.color)} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                  {content}
                </ParticleCard>
              )
            }
            ...
            return (
              <div key={index} className={baseCardClass} style={cardStyle(card.color)}>
                {content}
              </div>
            )
          })}
